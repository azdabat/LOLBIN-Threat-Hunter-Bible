# IR Playbook – cmstp.exe

## 1. L2 Workflow

- Confirm with IT / platform owners whether any legitimate usage of cmstp.exe is expected.
- Validate time, host, and user against scheduled maintenance or change windows.
- Gather:
  - Full command line for cmstp.exe.
  - Parent and first‑level children.
  - Any other alerts on same host/user in the last 7 days.
- Flag the case with a `LOLBIN-CMSTP` tag and assign to L3.
...
- Where feasible, constrain usage of cmstp.exe to specific contexts via:
