# IR Playbook – cscript.exe

## 1. L2 Workflow

- Confirm with IT / platform owners whether any legitimate usage of cscript.exe is expected.
- Validate time, host, and user against scheduled maintenance or change windows.
- Gather:
  - Full command line for cscript.exe.
  - Parent and first‑level children.
  - Any other alerts on same host/user in the last 7 days.
- Flag the case with a `LOLBIN-CSCRIPT` tag and assign to L3.
...
- Where feasible, constrain usage of cscript.exe to specific contexts via:
