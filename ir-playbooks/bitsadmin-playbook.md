...
- Gather:
  - Full command line for bitsadmin.exe.
  - Parent and first‑level children.
  - Any other alerts on same host/user in the last 7 days.
- Flag the case with a `LOLBIN-BITSADMIN` tag and assign to L3.
...
- Where feasible, constrain usage of bitsadmin.exe to specific contexts via:
