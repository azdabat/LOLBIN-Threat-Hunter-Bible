# IR Playbook – powershell.exe

## 1. L2 Workflow

- Confirm with IT / platform owners whether any legitimate usage of powershell.exe is expected.
- Validate time, host, and user against scheduled maintenance or change windows.
- Gather:
  - Full command line for powershell.exe.
  - Parent and first‑level children.
  - Any other alerts on same host/user in the last 7 days.
- Flag the case with a `LOLBIN-POWERSHELL` tag and assign to L3.
...
- Where feasible, constrain usage of powershell.exe to specific contexts via:
