# IR Playbook – mshta.exe

## 1. L2 Workflow

- Confirm with IT / platform owners whether any legitimate usage of mshta.exe is expected.
- Validate time, host, and user against scheduled maintenance or change windows.
- Gather:
  - Full command line for mshta.exe.
  - Parent and first‑level children.
  - Any other alerts on same host/user in the last 7 days.
- Flag the case with a `LOLBIN-MSHTA` tag and assign to L3.
...
- Where feasible, constrain usage of mshta.exe to specific contexts via:
