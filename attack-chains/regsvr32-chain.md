# regsvr32.exe Attack Chain Context
...
Loader / script stage (powershell.exe / wscript.exe / cscript.exe / mshta.exe)
    ↓
regsvr32.exe invoked with attacker‑controlled arguments
    ↓
...
where regsvr32.exe usually appears in relation to other technique categories.
...
- **Loader and LOLBIN stage**
  - `DeviceProcessEvents`:
    - Office or browser spawning loaders, scripts, or directly spawning regsvr32.exe.
  - `DeviceFileEvents`:
...
- Decide whether regsvr32.exe is:
