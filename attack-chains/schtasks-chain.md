# schtasks.exe Attack Chain Context
...
Loader / script stage (powershell.exe / wscript.exe / cscript.exe / mshta.exe)
    ↓
schtasks.exe invoked with attacker‑controlled arguments
    ↓
...
where schtasks.exe usually appears in relation to other technique categories.
...
- **Loader and LOLBIN stage**
  - `DeviceProcessEvents`:
    - Office or browser spawning loaders, scripts, or directly spawning schtasks.exe.
  - `DeviceFileEvents`:
...
- Decide whether schtasks.exe is:
