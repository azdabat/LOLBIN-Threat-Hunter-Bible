# rundll32.exe Attack Chain Context
...
Loader / script stage (powershell.exe / wscript.exe / cscript.exe / mshta.exe)
    ↓
rundll32.exe invoked with attacker‑controlled arguments
    ↓
...
where rundll32.exe usually appears in relation to other technique categories.
...
- **Loader and LOLBIN stage**
  - `DeviceProcessEvents`:
    - Office or browser spawning loaders, scripts, or directly spawning rundll32.exe.
  - `DeviceFileEvents`:
...
- Decide whether rundll32.exe is:
