# vssadmin.exe Attack Chain Context
...
Loader / script stage (powershell.exe / wscript.exe / cscript.exe / mshta.exe)
    ↓
vssadmin.exe invoked with attacker‑controlled arguments
    ↓
...
where vssadmin.exe usually appears in relation to other technique categories.
...
- **Loader and LOLBIN stage**
  - `DeviceProcessEvents`:
    - Office or browser spawning loaders, scripts, or directly spawning vssadmin.exe.
  - `DeviceFileEvents`:
...
- Decide whether vssadmin.exe is:
