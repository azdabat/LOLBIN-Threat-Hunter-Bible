# certutil.exe Attack Chain Context
...
Loader / script stage (powershell.exe / wscript.exe / cscript.exe / mshta.exe)
    ↓
certutil.exe invoked with attacker‑controlled arguments
    ↓
...
where certutil.exe usually appears in relation to other technique categories.
...
- **Loader and LOLBIN stage**
  - `DeviceProcessEvents`:
    - Office or browser spawning loaders, scripts, or directly spawning certutil.exe.
  - `DeviceFileEvents`:
...
- Decide whether certutil.exe is:
