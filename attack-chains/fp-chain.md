...
Loader / script stage (powershell.exe / wscript.exe / cscript.exe / mshta.exe)
    ↓
fp.exe invoked with attacker‑controlled arguments
    ↓
...
where fp.exe usually appears in relation to other technique categories.
...
- **Loader and LOLBIN stage**
  - `DeviceProcessEvents`:
    - Office or browser spawning loaders, scripts, or directly spawning fp.exe.
  - `DeviceFileEvents`:
