# wevtutil.exe Attack Chain Context
...
Loader / script stage (powershell.exe / wscript.exe / cscript.exe / mshta.exe)
    ↓
wevtutil.exe invoked with attacker‑controlled arguments
    ↓
...
where wevtutil.exe usually appears in relation to other technique categories.
...
- **Loader and LOLBIN stage**
  - `DeviceProcessEvents`:
    - Office or browser spawning loaders, scripts, or directly spawning wevtutil.exe.
  - `DeviceFileEvents`:
...
- Decide whether wevtutil.exe is:
