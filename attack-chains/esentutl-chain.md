# esentutl.exe Attack Chain Context
...
Loader / script stage (powershell.exe / wscript.exe / cscript.exe / mshta.exe)
    ↓
esentutl.exe invoked with attacker‑controlled arguments
    ↓
...
where esentutl.exe usually appears in relation to other technique categories.
...
- **Loader and LOLBIN stage**
  - `DeviceProcessEvents`:
    - Office or browser spawning loaders, scripts, or directly spawning esentutl.exe.
  - `DeviceFileEvents`:
...
- Decide whether esentutl.exe is:
