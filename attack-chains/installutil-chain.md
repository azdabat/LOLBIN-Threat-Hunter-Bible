# installutil.exe Attack Chain Context
...
Loader / script stage (powershell.exe / wscript.exe / cscript.exe / mshta.exe)
    ↓
installutil.exe invoked with attacker‑controlled arguments
    ↓
...
where installutil.exe usually appears in relation to other technique categories.
...
- **Loader and LOLBIN stage**
  - `DeviceProcessEvents`:
    - Office or browser spawning loaders, scripts, or directly spawning installutil.exe.
  - `DeviceFileEvents`:
...
- Decide whether installutil.exe is:
