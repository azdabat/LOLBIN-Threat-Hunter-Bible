# powershell.exe Attack Chain Context
...
Loader / script stage (powershell.exe / wscript.exe / cscript.exe / mshta.exe)
    ↓
powershell.exe invoked with attacker‑controlled arguments
    ↓
...
where powershell.exe usually appears in relation to other technique categories.
...
- **Loader and LOLBIN stage**
  - `DeviceProcessEvents`:
    - Office or browser spawning loaders, scripts, or directly spawning powershell.exe.
  - `DeviceFileEvents`:
...
- Decide whether powershell.exe is:
