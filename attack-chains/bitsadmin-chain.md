# bitsadmin.exe Attack Chain Context
...
Loader / script stage (powershell.exe / wscript.exe / cscript.exe / mshta.exe)
    ↓
bitsadmin.exe invoked with attacker‑controlled arguments
    ↓
...
where bitsadmin.exe usually appears in relation to other technique categories.
...
- **Loader and LOLBIN stage**
  - `DeviceProcessEvents`:
    - Office or browser spawning loaders, scripts, or directly spawning bitsadmin.exe.
  - `DeviceFileEvents`:
...
- Decide whether bitsadmin.exe is:
