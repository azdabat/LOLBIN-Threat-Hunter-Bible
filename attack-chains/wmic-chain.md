...
Loader / script stage (powershell.exe / wscript.exe / cscript.exe / mshta.exe)
    ↓
wmic.exe invoked with attacker‑controlled arguments
    ↓
...
where wmic.exe usually appears in relation to other technique categories.
...
- **Loader and LOLBIN stage**
  - `DeviceProcessEvents`:
    - Office or browser spawning loaders, scripts, or directly spawning wmic.exe.
  - `DeviceFileEvents`:
