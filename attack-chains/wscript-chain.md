# wscript.exe Attack Chain Context
...
Loader / script stage (powershell.exe / wscript.exe / cscript.exe / mshta.exe)
    ↓
wscript.exe invoked with attacker‑controlled arguments
    ↓
...
where wscript.exe usually appears in relation to other technique categories.
...
- **Loader and LOLBIN stage**
  - `DeviceProcessEvents`:
    - Office or browser spawning loaders, scripts, or directly spawning wscript.exe.
  - `DeviceFileEvents`:
