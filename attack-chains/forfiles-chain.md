# forfiles.exe Attack Chain Context
...
Loader / script stage (powershell.exe / wscript.exe / cscript.exe / mshta.exe)
    ↓
forfiles.exe invoked with attacker‑controlled arguments
    ↓
...
where forfiles.exe usually appears in relation to other technique categories.
...
- **Loader and LOLBIN stage**
  - `DeviceProcessEvents`:
    - Office or browser spawning loaders, scripts, or directly spawning forfiles.exe.
  - `DeviceFileEvents`:
...
- Decide whether forfiles.exe is:
