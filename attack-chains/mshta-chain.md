# mshta.exe Attack Chain Context
...
Loader / script stage (powershell.exe / wscript.exe / cscript.exe / mshta.exe)
    ↓
mshta.exe invoked with attacker‑controlled arguments
    ↓
...
where mshta.exe usually appears in relation to other technique categories.
...
- **Loader and LOLBIN stage**
  - `DeviceProcessEvents`:
    - Office or browser spawning loaders, scripts, or directly spawning mshta.exe.
  - `DeviceFileEvents`:
