# cscript.exe Attack Chain Context
...
Loader / script stage (powershell.exe / wscript.exe / cscript.exe / mshta.exe)
    ↓
cscript.exe invoked with attacker‑controlled arguments
    ↓
...
where cscript.exe usually appears in relation to other technique categories.
...
- **Loader and LOLBIN stage**
  - `DeviceProcessEvents`:
    - Office or browser spawning loaders, scripts, or directly spawning cscript.exe.
  - `DeviceFileEvents`:
