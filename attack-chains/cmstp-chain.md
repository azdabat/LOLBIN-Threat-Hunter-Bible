# cmstp.exe Attack Chain Context
...
Loader / script stage (powershell.exe / wscript.exe / cscript.exe / mshta.exe)
    ↓
cmstp.exe invoked with attacker‑controlled arguments
    ↓
...
where cmstp.exe usually appears in relation to other technique categories.
...
- **Loader and LOLBIN stage**
  - `DeviceProcessEvents`:
    - Office or browser spawning loaders, scripts, or directly spawning cmstp.exe.
  - `DeviceFileEvents`:
