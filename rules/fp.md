...
fp.exe is a legitimate Windows binary. In modern intrusion tradecraft it is abused to move
execution into a signed, trusted process. This evades naïve allow‑lists and simplistic EDR rules.

Abuse patterns for fp.exe in the last few years include:
...
Effective detection focuses on **context and behaviour**, not on treating fp.exe as inherently malicious.
...
1. **Expand process context**
   ...
   - Build an execution graph: parent → fp.exe → children.
...
- Capture **all legitimate** fp.exe usage for at least 14–30 days.
- Document:
  - Parents,
  - Command lines,
